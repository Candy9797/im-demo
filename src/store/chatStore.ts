...
export const useChatStore = create<ChatState>()(
  // persist 是 Zustand 的持久化中间件，用来在本地存储中保存和恢复状态
  // 每次 store 更新时：把选中的部分 state 序列化，写入 storage，
  // 页面加载时：从 storage 读出，反序列化后写回 store
  persist(
    immer((set, get) => ({
      ...
      connectAsGuest: async () => {
        ...
        try {
          ...
          const msg =
            e instanceof Error ? e.message : "Guest connection failed";
      ...
      initialize: async () => {
        ...
        set({ authError: null, wantFreshStart: false });
        ...
        client.on("server_error", (payload: unknown) => {
          const p = payload as { code?: string; message?: string; messageId?: string };
          const toastMsg = typeof p.message === "string" ? p.message : "操作失败，请重试";
          if (p.code === "recall_expired" && p.messageId && _pendingRecallRevert?.messageId === p.messageId) {
    ...
    })),
    {
      ...
      // storage：使用 chatPersistStorage（IndexedDB），而不是默认的 localStorage
      storage: createJSONStorage(() => chatPersistStorage),
      // partialize：决定要持久化的字段
      // 只持久化 messages 和 conversationId，其他字段都不持久化
      //       不持久化的字段	原因
      // client	IMClient 实例，不可序列化，重新连接时会新建
      // auth	token 敏感，且会过期，每次需重新登录
      // connectionState, phase 等	运行时状态，刷新后需重新与服务端同步
      // isOpen, isMinimized 等 UI 状态	一般刷新后重置即可
      // quoteTarget, scrollToInputRequest	临时 UI 状态，不必持久化

      //       持久化 messages 和 conversationId 是为了：
      // 刷新或关闭后仍能看到最近消息
      // 知道当前会话 id，方便重新建立会话、恢复历史
      partialize: (s) => ({
        messages: s.messages,
...
// ┌─────────────────────────────────────────────────────────────┐
// │  chatStore 更新（例如收到新消息）                              │
// └─────────────────────────────────────────────────────────────┘
//                               │
//                               ▼
// ┌─────────────────────────────────────────────────────────────┐
// │  persist 调用 partialize(s)                                  │
// │  → 得到 { messages, conversationId }                         │
// └─────────────────────────────────────────────────────────────┘
//                               │
//                               ▼
// ┌─────────────────────────────────────────────────────────────┐
